...
## v.2.1.5 - 24.05.2024
- **Redesign:** With this release, we're introducing the new design of the mogenius platform. We're providing an  enhanced dark theme as the default and a distinctively improved light mode. The new themes now cater towards the specific use cases of dark and light mode even better. But most importantly, we find they just look great and we hope you feel the same!
...
- **Breadcrump navigation:** To enhance navigation through projects and services, we introduced a global breadcrumb system that allows you to quickly move through all organizational levels within mogenius.
- **Storage 2.0:** The NFS storage browser has received a major update, improving the overall stability when working with volumes, folders, and files. Additionally, we've introduced enhanced monitoring for the capacity and status of a volume, aggregating the health of NFS pod, persistent volume, and persistent volume claim from Kubernetes. It is now also possible to change the size of a volume during its creation.
