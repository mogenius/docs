...
The pipeline starter is based on GitHub Actions workflow templates maintained by mogenius. You can find these templates in the [mogenius build-workflows repository](https://github.com/mogenius/build-workflows). The templates can also be used independently in your own CI/CD pipelines for custom integrations with the mogenius operator.

<Tip>
For production deployments, consider enabling [GitOps](/cluster-management/gitops) to layer declarative Git-based management on top of your CI/CD pipeline. This gives you version-controlled infrastructure with full audit history.
</Tip>